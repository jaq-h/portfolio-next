      ...
      {ProjectData.projects &&
        ProjectData.projects.map((project, index) => {
          return (
            <div
              key={`project-${index}`}
              className="md:flex md:justify-between grid grid-cols-1 mb-10 py-8 bg-slate-800 border-2 border-slate-500 rounded-md "
            >
                    ...
                    project.projectLinks.map((link, index) => {
                      return (
                        <span className="pr-4" key={`project-link-${index}`}>
                          <Link
                            className="p-2 bg-slate-900  hover:border-slate-400 border-2 border-slate-500 rounded-md"
                            href={link.link}
                          >
                            <Icon icon={link.icon} variant="ui" /> {link.title}
                          </Link>
                        </span>
                      );
                    })}
